...
include_once(dirname(__FILE__).'/class/weixin_baoming.class.php');

$objWebInit = new weixin_baoming();
$objWebInit->db();
...
include($strFilename);
$arrGMeta['weixin_baoming']['name'] = '微信_预约报名';
$arrGMeta['weixin_baoming']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['weixin_baoming']['admin'] = array(
								array(
...
$arrGMeta['weixin_baoming']['meta'] = array(
									  'Title' => $arrGMeta['weixin_baoming']['name'],
									  'Description' => $arrGMeta['weixin_baoming']['name'],
									  'Keywords' => $arrGMeta['weixin_baoming']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."weixin_baoming`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."weixin_baoming` (
...
) ENGINE=MyISAM {$extend} COMMENT='微信_预约报名表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."weixin_baoming_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."weixin_baoming_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='微信_预约报名分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('微信_预约报名系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>