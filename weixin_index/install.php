...
include_once(dirname(__FILE__).'/class/weixin_index.class.php');
...
include($strFilename);
$arrGMeta['weixin_index']['name'] = '微信首页';
$arrGMeta['weixin_index']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['weixin_index']['admin'] = array(
								array(
...
$arrGMeta['weixin_index']['meta'] = array(
									  'Title' => $arrGMeta['weixin_index']['name'],
									  'Description' => $arrGMeta['weixin_index']['name'],
									  'Keywords' => $arrGMeta['weixin_index']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."weixin_index`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."weixin_index` (
...
) ENGINE=MyISAM {$extend} COMMENT='微信首页表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."weixin_index_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."weixin_index_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='微信首页分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('微信首页系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>