...
include_once(dirname(__FILE__).'/class/xingfu_teaching.class.php');

$objWebInit = new xingfu_teaching();
$objWebInit->db();
...
include($strFilename);
$arrGMeta['xingfu_teaching']['name'] = '武术教学';
$arrGMeta['xingfu_teaching']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_teaching']['admin'] = array(
								array(
...
$arrGMeta['xingfu_teaching']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_teaching']['name'],
									  'Description' => $arrGMeta['xingfu_teaching']['name'],
									  'Keywords' => $arrGMeta['xingfu_teaching']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_teaching`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_teaching` (
...
) ENGINE=MyISAM {$extend} COMMENT='武术教学表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_teaching_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_teaching_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='武术教学分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('武术教学系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>