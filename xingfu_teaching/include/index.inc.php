 ...
 * @version		$Id$
 * @package		ArthurXF
 * @subpackage	xingfu_teaching
 */
require_once('config/config.inc.php');
require_once("class/xingfu_teaching.class.php");
...
$arrInfoList = $objWebInit->getInfoList($strWhere,' ORDER BY topflag DESC,submit_date DESC',($intPage-1)*$arrGPage['page_size'],$arrGPage['page_size'],'id,type_id,title,thumbnail,structon_tb,submit_date,bedeck,summary');
...
for($i = 0 ;$i < count($arrInfoListTmp) - 1; $i++){
    $TmpVideo[$i] = $arrInfoListTmp[$i]['intro'];
}

for($i = 0 ;$i < count($arrInfoList) - 1; $i++){
    $arrInfoList[$i]['intro'] = $TmpVideo[$i] ;
}
...
$arrMOutput["smarty_assign"]['arrGWeb']['css'][] = 'xingfu_teaching.css';
