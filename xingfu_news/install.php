...
include_once(dirname(__FILE__).'/class/xingfu_news.class.php');

$objWebInit = new xingfu_news();
$objWebInit->db();
...
include($strFilename);
$arrGMeta['xingfu_news']['name'] = '兴甫新闻';
$arrGMeta['xingfu_news']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_news']['admin'] = array(
								array(
...
$arrGMeta['xingfu_news']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_news']['name'],
									  'Description' => $arrGMeta['xingfu_news']['name'],
									  'Keywords' => $arrGMeta['xingfu_news']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_news`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_news` (
...
) ENGINE=MyISAM {$extend} COMMENT='兴甫新闻表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_news_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_news_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='兴甫新闻分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('兴甫新闻系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>