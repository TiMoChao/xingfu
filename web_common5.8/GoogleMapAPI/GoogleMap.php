...
define('EARTH_RADIUS',6378137); //赤道半径 earth radius
define('POLAR_RADIUS',6356725); //极半径 polar radius
...
class GoogleMapAPI {
	...
	*/
     ...
     */
	 function geoGetBox($lat,$lon,$offset=2,$angle) {
		 $dx = $offset*1000*sin($angle*PI/180);
		 $dy = $offset*1000*cos($angle*PI/180);
		 $arr['lat']  = number_format(($dy / $this->Ec($lat) + $this->RadLat($lat)) * 180 / PI,6);
		 $arr['lon'] = number_format(($dx / $this->Ed($lat) + $this->RadLon($lon)) * 180 / PI,6);		 
		 return $arr;
	 }
	 ...
	 * 判断经纬度是否在多边形范围之内
	 * @author		肖飞
	 * @param string $point	经纬度点例如'30 10'
	 * @param array $polygon	经纬度多边形例如array("10 0", "20 0", "30 10", "30 20", "20 30", "10 30", "0 20", "0 10", "10 0");
	 * @return int
	 */
	 ...
	 * @param string	$pointString
	 */
    function pointCoordinatespointToString($pointArray) {
		$coordinates = '';
		foreach($pointArray as $v){
			if($coordinates != '') $coordinates.= ' '.$v;
			else $coordinates .= $v;
		}
        return $coordinates;
    }
	 ...
	 * @author		何军毅
	 * @param string	$pointString
	 * @param array $checkCode //要检测的地址的坐标
	 * @param array $areaCode //要生成检查区域地址的坐标
	 * @param int $offset 生成区域的偏移量
	 */
    function isInPolygon($checkCode,$areaCode,$offset=2){
    	$checkCode = $this->pointCoordinatespointToString($checkCode);
    	
    	$code45 = $this->geoGetBox($areaCode['lat'],$areaCode['lon'],$offset,45);
    	$code45String = $this->pointCoordinatespointToString($code45);
    	$polygon[] = $code45String;
    	
    	$code135 = $this->geoGetBox($areaCode['lat'],$areaCode['lon'],$offset,135);
    	$code135String = $this->pointCoordinatespointToString($code135);
    	$polygon[]=$code135String;
    	
    	$code225 = $this->geoGetBox($areaCode['lat'],$areaCode['lon'],$offset,225);
    	$code225String = $this->pointCoordinatespointToString($code225);
    	$polygon[]=$code225String;
    	
    	$code315 = $this->geoGetBox($areaCode['lat'],$areaCode['lon'],$offset,315);
    	$code315String = $this->pointCoordinatespointToString($code315);
    	$polygon[]=$code315String;

    	
    	$reslt = $this->pointInPolygon($checkCode, $polygon);
    	return $reslt;
    }
}
?>
