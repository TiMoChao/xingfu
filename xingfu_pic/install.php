...
include_once(dirname(__FILE__).'/class/xingfu_pic.class.php');
...
include($strFilename);
$arrGMeta['xingfu_pic']['name'] = '首页滑动图片';
$arrGMeta['xingfu_pic']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_pic']['admin'] = array(
								array(
...
$arrGMeta['xingfu_pic']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_pic']['name'],
									  'Description' => $arrGMeta['xingfu_pic']['name'],
									  'Keywords' => $arrGMeta['xingfu_pic']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_pic`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_pic` (
...
) ENGINE=MyISAM {$extend} COMMENT='首页滑动图片表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_pic_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_pic_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='首页滑动图片分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('首页滑动图片系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>