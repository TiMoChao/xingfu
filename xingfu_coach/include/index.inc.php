 ...
 * @version		$Id$
 * @package		ArthurXF
 * @subpackage	xingfu_coach
 */
require_once('config/config.inc.php');
require_once("class/xingfu_coach.class.php");
...
$arrInfoList = $objWebInit->getInfoList($strWhere,' ORDER BY topflag DESC,submit_date DESC',($intPage-1)*$arrGPage['page_size'],$arrGPage['page_size'],'id,type_id,title,submit_date,thumbnail,bedeck,summary');
...
for($i = 0 ;$i < count($arrInfoListTmp) - 1; $i++){
    $TmpIntro[$i] = $arrInfoListTmp[$i]['intro'];
}

for($i = 0 ;$i < count($arrInfoList) - 1; $i++){
    $arrInfoList[$i]['intro'] = $TmpIntro[$i] ;
}
...
$arrMOutput["smarty_assign"]['arrGWeb']['css'][] = 'xingfu_coach.css';
