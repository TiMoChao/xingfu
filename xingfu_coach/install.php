...
include_once(dirname(__FILE__).'/class/xingfu_coach.class.php');

$objWebInit = new xingfu_coach();
$objWebInit->db();
...
include($strFilename);
$arrGMeta['xingfu_coach']['name'] = '名师风采';
$arrGMeta['xingfu_coach']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_coach']['admin'] = array(
								array(
...
$arrGMeta['xingfu_coach']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_coach']['name'],
									  'Description' => $arrGMeta['xingfu_coach']['name'],
									  'Keywords' => $arrGMeta['xingfu_coach']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_coach`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_coach` (
...
) ENGINE=MyISAM {$extend} COMMENT='名师风采表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_coach_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_coach_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='名师风采分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('名师风采系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>