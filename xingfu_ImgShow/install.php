...
include($strFilename);
$arrGMeta['xingfu_ImgShow']['name'] = '兴甫风采';
$arrGMeta['xingfu_ImgShow']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_ImgShow']['admin'] = array(
								array(
...
$arrGMeta['xingfu_ImgShow']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_ImgShow']['name'],
									  'Description' => $arrGMeta['xingfu_ImgShow']['name'],
									  'Keywords' => $arrGMeta['xingfu_ImgShow']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_ImgShow`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_ImgShow` (
...
) ENGINE=MyISAM {$extend} COMMENT='兴甫风采表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_ImgShow_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_ImgShow_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='兴甫风采分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('兴甫风采系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>