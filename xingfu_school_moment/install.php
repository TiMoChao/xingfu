...
include_once(dirname(__FILE__).'/class/xingfu_school_moment.class.php');
...
include($strFilename);
$arrGMeta['xingfu_school_moment']['name'] = '兴甫幼儿园_ 精彩瞬间';
$arrGMeta['xingfu_school_moment']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_school_moment']['admin'] = array(
								array(
...
$arrGMeta['xingfu_school_moment']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_school_moment']['name'],
									  'Description' => $arrGMeta['xingfu_school_moment']['name'],
									  'Keywords' => $arrGMeta['xingfu_school_moment']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_moment`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_moment` (
...
) ENGINE=MyISAM {$extend} COMMENT='兴甫幼儿园_ 精彩瞬间表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_moment_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_moment_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='兴甫幼儿园_ 精彩瞬间分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('兴甫幼儿园_ 精彩瞬间系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>