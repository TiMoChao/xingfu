...
include_once(dirname(__FILE__).'/class/xingfu_school_teacher.class.php');

$objWebInit = new xingfu_school_teacher();
$objWebInit->db();
...
include($strFilename);
$arrGMeta['xingfu_school_teacher']['name'] = '兴甫幼儿园_ 园丁风采';
$arrGMeta['xingfu_school_teacher']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_school_teacher']['admin'] = array(
								array(
...
$arrGMeta['xingfu_school_teacher']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_school_teacher']['name'],
									  'Description' => $arrGMeta['xingfu_school_teacher']['name'],
									  'Keywords' => $arrGMeta['xingfu_school_teacher']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_teacher`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_teacher` (
...
) ENGINE=MyISAM {$extend} COMMENT='兴甫幼儿园_ 园丁风采表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_teacher_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_teacher_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='兴甫幼儿园_ 园丁风采分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('兴甫幼儿园_ 园丁风采系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>