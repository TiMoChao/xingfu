...
include_once(dirname(__FILE__).'/class/weixin_teachers.class.php');
...
include($strFilename);
$arrGMeta['weixin_teachers']['name'] = '微信名教风采';
$arrGMeta['weixin_teachers']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['weixin_teachers']['admin'] = array(
								array(
...
$arrGMeta['weixin_teachers']['meta'] = array(
									  'Title' => $arrGMeta['weixin_teachers']['name'],
									  'Description' => $arrGMeta['weixin_teachers']['name'],
									  'Keywords' => $arrGMeta['weixin_teachers']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."weixin_teachers`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."weixin_teachers` (
...
) ENGINE=MyISAM {$extend} COMMENT='微信名教风采表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."weixin_teachers_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."weixin_teachers_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='微信名教风采分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('微信名教风采系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>