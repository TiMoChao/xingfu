...
if(strpos($strMyUrl,$arrGWeb['file_suffix'])){
	...
	if(!empty($arrTemp[1]) && $arrTemp[1] == substr($arrGWeb['cache_url'],1)){
		...
		if($arrTemp[2] == 'archives') $strContents = file_get_contents('http://'.$_SERVER['HTTP_HOST'].'/'.$arrTemp[2].'/detail.php?name='.$intID);
		else $strContents = file_get_contents('http://'.$_SERVER['HTTP_HOST'].'/'.$arrTemp[2].'/detail.php?id='.$intID);
		if($strContents){
			echo $strContents;
		}
		$objCache->cache_end();
