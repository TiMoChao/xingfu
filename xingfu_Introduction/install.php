...
include_once(dirname(__FILE__).'/class/xingfu_Introduction.class.php');
...
include($strFilename);
$arrGMeta['xingfu_Introduction']['name'] = '兴甫简介';
$arrGMeta['xingfu_Introduction']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_Introduction']['admin'] = array(
								array(
...
$arrGMeta['xingfu_Introduction']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_Introduction']['name'],
									  'Description' => $arrGMeta['xingfu_Introduction']['name'],
									  'Keywords' => $arrGMeta['xingfu_Introduction']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_Introduction`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_Introduction` (
...
) ENGINE=MyISAM {$extend} COMMENT='兴甫简介表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_Introduction_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_Introduction_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='兴甫简介分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('兴甫简介系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>