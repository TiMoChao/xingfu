...
for($i=1;$i<=$intDays;$i++){
	...
	$strWhere="where time >= '".$intStart."' and time<= '".$intEnd."' and trade_type='3' and money='0.1' ";
	$strSQL = "SELECT count(distinct(user_id)) as '".date("Y-m-d", $intTmp)."' FROM ".$arrGWeb['db_tablepre']."usermoney ".$strWhere;
...
$strChart.='}';
// 输出到模板
$arrMOutput["smarty_assign"]['strNav'] = '用户登录量分析';
...
$arrMOutput["smarty_assign"]['MAIN'] = $arrGSmarty['admin_main_dir'].'count/login_count.htm';
