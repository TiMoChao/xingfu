...
for($i=1;$i<=$intDays;$i++){
	...
	$strSQL = "SELECT count(user_id) as '".date("Y-m-d", $intTmp)."' FROM ".$arrGWeb['db_tablepre']."user ".$strWhere;	
...
$strChart.='}';
// 输出到模板
$arrMOutput["smarty_assign"]['strNav'] = '用户注册量分析';
...
$arrMOutput["smarty_assign"]['MAIN'] = $arrGSmarty['admin_main_dir'].'count/reg_count.htm';
