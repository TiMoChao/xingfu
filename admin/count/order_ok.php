...
for($i=1;$i<=$intDays;$i++){
	...
	$strWhere="where submit_date > '".date("Y-m-d", $intDate)." 24:00:00' and submit_date< '".date("Y-m-d", $intTmp)." 24:00:00' and status in (2,4,7,8)";
	$strSQL = "SELECT count(order_id) as '".date("Y-m-d", $intTmp)."' FROM ".$arrGWeb['db_tablepre']."order ".$strWhere;
...
$strChart.='}';
// 输出到模板
$arrMOutput["smarty_assign"]['strNav'] = '订单成交量分析';
...
$arrMOutput["smarty_assign"]['MAIN'] = $arrGSmarty['admin_main_dir'].'count/order_ok.htm';
