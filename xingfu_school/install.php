...
include_once(dirname(__FILE__).'/class/xingfu_school.class.php');

$objWebInit = new xingfu_school();
$objWebInit->db();
...
include($strFilename);
$arrGMeta['xingfu_school']['name'] = '兴甫幼儿园';
$arrGMeta['xingfu_school']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_school']['admin'] = array(
								array(
...
$arrGMeta['xingfu_school']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_school']['name'],
									  'Description' => $arrGMeta['xingfu_school']['name'],
									  'Keywords' => $arrGMeta['xingfu_school']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school` (
...
) ENGINE=MyISAM {$extend} COMMENT='兴甫幼儿园表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='兴甫幼儿园分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('兴甫幼儿园系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>