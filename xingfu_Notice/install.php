...
include_once(dirname(__FILE__).'/class/xingfu_Notice.class.php');
...
include($strFilename);
$arrGMeta['xingfu_Notice']['name'] = '网站公告';
$arrGMeta['xingfu_Notice']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_Notice']['admin'] = array(
								array(
...
$arrGMeta['xingfu_Notice']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_Notice']['name'],
									  'Description' => $arrGMeta['xingfu_Notice']['name'],
									  'Keywords' => $arrGMeta['xingfu_Notice']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_Notice`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_Notice` (
...
) ENGINE=MyISAM {$extend} COMMENT='网站公告表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_Notice_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_Notice_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='网站公告分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('网站公告系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>