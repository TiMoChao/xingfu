<?php
/**
 * 兴甫幼儿园_ 基本课程后台管理栏目修改文件
 *
 * @author		Arthur(ArthurXF@gmail.com)
 * @copyright	(c) 2006 by bizeway.com
 * @version		$Id$
 * @package		ArthurXF
 * @subpackage	xingfu_school_Basic_courses
 */
require_once('../config/config.inc.php');
require_once("../class/xingfu_school_Basic_courses.class.php");
...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if(!empty($_POST['savevideo'])){
		foreach($_POST['savevideo'] as $key => $val){
			$_POST['video'][$key] = array('video'=>$val,'video_link'=>$_POST['video_link'.$key],'videopic'=>$_POST['savevideopic'][$key],'video_narrate'=>$_POST['video_narrate'.$key],'video_title'=>$_POST['video_title'.$key],'video_time'=>$_POST['video_time'.$key]);
		}
	}
