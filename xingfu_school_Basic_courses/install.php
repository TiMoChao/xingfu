...
include_once(dirname(__FILE__).'/class/xingfu_school_Basic_courses.class.php');

$objWebInit = new xingfu_school_Basic_courses();
$objWebInit->db();
...
include($strFilename);
$arrGMeta['xingfu_school_Basic_courses']['name'] = '兴甫幼儿园_ 基本课程';
$arrGMeta['xingfu_school_Basic_courses']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_school_Basic_courses']['admin'] = array(
								array(
...
$arrGMeta['xingfu_school_Basic_courses']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_school_Basic_courses']['name'],
									  'Description' => $arrGMeta['xingfu_school_Basic_courses']['name'],
									  'Keywords' => $arrGMeta['xingfu_school_Basic_courses']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_Basic_courses`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_Basic_courses` (
...
) ENGINE=MyISAM {$extend} COMMENT='兴甫幼儿园_ 基本课程表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_Basic_courses_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_Basic_courses_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='兴甫幼儿园_ 基本课程分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('兴甫幼儿园_ 基本课程系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>