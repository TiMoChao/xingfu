...
include_once(dirname(__FILE__).'/class/xingfu_school_class.class.php');

$objWebInit = new xingfu_school_class();
$objWebInit->db();
...
include($strFilename);
$arrGMeta['xingfu_school_class']['name'] = '兴甫幼儿园_ 班级信息';
$arrGMeta['xingfu_school_class']['cache'] = 1;//该栏目是否生成静态页，0=不可以，1=可以
$arrGMeta['xingfu_school_class']['admin'] = array(
								array(
...
$arrGMeta['xingfu_school_class']['meta'] = array(
									  'Title' => $arrGMeta['xingfu_school_class']['name'],
									  'Description' => $arrGMeta['xingfu_school_class']['name'],
									  'Keywords' => $arrGMeta['xingfu_school_class']['name'],
									);
...
//sql语句
if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_class`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_class` (
...
) ENGINE=MyISAM {$extend} COMMENT='兴甫幼儿园_ 班级信息表' ;";

if($_SESSION['install_type']) $sql[] = "DROP TABLE IF EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_class_type`;";
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."xingfu_school_class_type` (
...
) ENGINE=MyISAM  {$extend} COMMENT='兴甫幼儿园_ 班级信息分类表' ;";

foreach($sql as $val){
	$objWebInit->db->query($val);
}
if(empty($arrModule)){
	check::AlertExit('兴甫幼儿园_ 班级信息系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>